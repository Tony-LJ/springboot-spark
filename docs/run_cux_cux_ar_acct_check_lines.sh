...
create table if not exists bi_tmp.ods_cux_cux_inv_issue_line
USING org.apache.spark.sql.jdbc
OPTIONS (
    url "jdbc:oracle:thin:@ebsdb-scan.kinwong.com:1531/prod",
    dbtable "CUX.CUX_INV_ISSUE_LINE",
    driver "oracle.jdbc.driver.OracleDriver",
    user "hadoop",
    password "vSWnGLcdd8ch"
);

select * from bi_tmp.ods_cux_cux_inv_issue_line limit 100;
...
select * from bi_tmp.ods_cux_cux_inv_issue_line limit 100;
select count(1) from bi_tmp.ods_cux_cux_inv_issue_line limit 10;
"
